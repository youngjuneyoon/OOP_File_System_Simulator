...
int HierarchicalFileSystem::deleteFile(std::string path) {
	...
	*/
	...
	DirectoryFile* changeToDir = dynamic_cast<DirectoryFile*>(fileToDelete);

	if (changeToDir != nullptr) {
		if (fileToDelete->getSize() != 0) {
			cout << "Not able to delete because the directory is not empty" << endl;
